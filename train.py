...
import os
os.environ["PL_TORCH_DISTRIBUTED_BACKEND"] = "gloo"     #use gloo for distributed training for windows, default nccl for linux
...
class SequenceLightningModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        params = [p for p in all_params if not hasattr(p, "_optim")]    # Exclude special parameters
...
def create_trainer(config, **kwargs):
    ...
    if config.get("wandb") is not None:
        ...
        #instantiate a WandBLogger instance
        logger = WandbLogger(
