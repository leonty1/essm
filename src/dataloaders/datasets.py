...
default_data_path = Path(r'D:\backup\liangtongyi\Datasets')
...
class SequenceDataset:
    ...
    # contain a plain list of all subclasses in the entire inheritance tree. ##registry is a dict of all subclasses
...
class BIDMC(SequenceDataset):
    ...
    def setup(self):
        ...
        # y: (dataset_size)
        X_train = np.load(self.data_dir / BIDMC / self.target / split / "trainx.npy")
        y_train = np.load(self.data_dir / BIDMC / self.target / split / "trainy.npy")
        X_val = np.load(self.data_dir / BIDMC / self.target / split / "validx.npy")
        y_val = np.load(self.data_dir / BIDMC / self.target / split / "validy.npy")
        X_test = np.load(self.data_dir / BIDMC / self.target / split / "testx.npy")
        y_test = np.load(self.data_dir / BIDMC / self.target / split / "testy.npy")
...
class IMDB(SequenceDataset):
    ...
    def prepare_data(self):
        if self.cache_dir is None:  # Just download the dataset
            load_dataset(self._name_, cache_dir=self.data_dir)      # Download the dataset and cache it in data_dir/imdb
    ...
    def _save_to_cache(self, dataset, tokenizer, vocab, cache_dir):
        ...
        dataset.save_to_disk(str(cache_dir))        # Save the processed dataset to cache_dir/imdb
