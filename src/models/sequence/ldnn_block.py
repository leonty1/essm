...
from src.models.nn.components import Normalization, Activation
...
from src.models.nn.components import LinearActivation
...
class SequenceResidualBlock(SequenceModule):
    def __init__(
        ...
        self.layer = utils.instantiate(registry.layer, layer, d_input)      #层的初始化，不仅把layer的configs传进去，同时把输入的维度d_input传进去了
        ...
        #     self.out1 = LinearActivation(d_input=self.d_input, d_output=self.d_input, transposed=True)
        #     self.out2 = LinearActivation(d_input=self.d_input, d_output=self.d_input, transposed=True)
        ...
        # Dropout
        drop_cls = nn.Dropout1d if self.transposed else nn.Dropout
        # drop_cls = nn.Dropout
    ...
    def forward(self, x, *args, state=None, **kwargs):
        ...
        #ldnn as a black box module
        y, state = self.layer(y, *args, state=state, **kwargs)

        # Dropout and Residual
        y=self.drop(y)

        if self.residual is not None: x = self.residual(x, y, self.transposed)
