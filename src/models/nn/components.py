...
def LinearActivation(
    ...
    linear_cls = TransposedLinear if transposed else nn.Linear      #[B M L]时transposed=True使用转置的linear layer，[B L M]时 transposed=False使用默认的linear layer
    ...
    # Weight norm
    if weight_norm:
        linear = nn.utils.weight_norm(linear)       #一种加速网络训练的处理方式https://arxiv.org/abs/1602.07868
...
class TransposedLinear(nn.Module):
    ...
    def __init__(self, d_input, d_output, bias=True):
        ...
        if bias:
            ...
            bound = 1 / torch.sqrt(torch.tensor(d_input))
...
class Normalization(nn.Module):
    '''
    layer, instance, batch 给出名字，使用相应的norm
    '''
    def __init__(
        self,
        d,
        ...
        **kwargs
        ...
        if _name_ == 'layer':
            ...
            self.channel = False
            # norm_args = {'affine': True, 'track_running_stats': True, 'momentum':0.95}
